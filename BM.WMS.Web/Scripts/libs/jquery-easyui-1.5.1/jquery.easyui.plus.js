...
function dataBatchOperater(url, datagrid) {
    ...
    if (rows.length > 0) {
        ...
        $.post(url + "?id=" + ids, function (data) {
            ...
            $.msfiveseccond(index_lang_tip, data.message);

        }, "json");
    } else { $.msfiveseccond(index_lang_tip, app.localize("PleaseSelectTheOperatingRecord")); }
}
...
*/
$.extend({
    msthreesecond: function (msg, title) {
        title = title || app.localize('Tip');
        $.messager.show({
...
$.extend({
    msfiveseccond: function (msg, title) {
        title = title || app.localize('Tip');
        $.messager.show({
...
$.extend({
    mstensecond: function (msg, title) {
        title = title || app.localize('Tip');
        $.messager.show({
