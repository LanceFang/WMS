...
namespace BM.WMS.Localization
{
    ...
    public class LanguageAppService : WMSAppServiceBase, ILanguageAppService
    {
        ...
        public async Task<PagedResultOutput<ApplicationLanguageListDto>> GetLanguages()
        {
            var languages = (await _applicationLanguageManager.GetLanguagesAsync(AbpSession.TenantId)).OrderBy(l => l.DisplayName);

            int count = languages.Count();
            return new PagedResultOutput<ApplicationLanguageListDto>(
                        count,
        ...
        public async Task<PagedResultOutput<LanguageTextListDto>> GetLanguageTexts(GetLanguageTextsInput input)
        {
            ...
            //Ordering
            if (!input.sort.IsNullOrEmpty() && !input.order.IsNullOrEmpty())
            {
                languageTexts = languageTexts.OrderBy(string.Format(" {0} {1} ", input.sort, input.order));
            }
